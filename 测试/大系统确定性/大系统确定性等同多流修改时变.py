...
def main():
    ...
    # ==========================
    # 4. 计算 Heuristic SINR 与 功率估算 (严格映射双重循环逻辑)
    # ==========================
            ...
            for j in range(K):
                for s_prime in range(N_layer):
                    ...
                    if j == k and s_prime == s:
                        ...
                        interf_vk += p_alloc_flat[vj] * lambda_R_ks
                    else:
                        # Inter-user interference (其他用户的流)
                        interf_vk += p_alloc_flat[vj] * lambda_R_ks * np.trace(R_Tk_list[j] @ R_Tk_list[k]).real / Mt

            for t in range(T):
                aging_factor = rho_k[k] ** (2 * t)
                gain = aging_factor * large_scale[k] * p_alloc_flat[vk] * lambda_R_ks * Mt * (P/K_tot)
                mu_loss = (1 - aging_factor) * large_scale[k] * interf_vk * (P/K_tot)
