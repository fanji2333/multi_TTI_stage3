...
class P3O(PPO):
    ...
    def _loss_pi_cost(
        ...
        rc = ratio.unsqueeze(1) * adv_c
        rc_cliped = ratio_cliped.unsqueeze(1) * adv_c
        surr_cadv = torch.max(rc, rc_cliped)
        surr_cadv = surr_cadv.mean(dim=0)
        # Jc = torch.zeros((self._cfgs.env_cfgs.U,)).to(self._device)
        # for u in range(self._cfgs.env_cfgs.U):
        #     self._logger.store({f'Metrics/surr_cost_Adv{u}':surr_cadv[u].item()})
        #     Jc[u] = self._logger.get_stats(f'Metrics/EpCost{u}')[0] - self._cfgs.algo_cfgs.cost_limit
        self._logger.store({f'Metrics/surr_cost_Adv': surr_cadv.item()})
        # Jc = self._logger.get_stats(f'Metrics/EpCost')[0] - self._cfgs.algo_cfgs.cost_limit
        Jc = self._logger.get_stats(f'Metrics/EpCost', last_ep=True)[0] / self._logger.get_stats(f'Metrics/EpLen', last_ep=True)[0] - self._cfgs.algo_cfgs.cost_limit
        # loss_cost = self._cfgs.algo_cfgs.kappa * F.relu(surr_cadv + Jc)
        loss_cost = self._cfgs.algo_cfgs.kappa * F.relu(surr_cadv + (1 - self._cfgs.algo_cfgs.gamma) * Jc)
        # self._logger.store({'Loss/Loss_pi_cost': loss_cost.mean().item()})
        # return loss_cost.mean()
        # loss_c = torch.maximum(loss_cost, torch.zeros_like(loss_cost))
        self._logger.store({'Loss/Loss_pi_cost': loss_cost.sum().item()})
        return loss_cost.sum()
    ...
    def _update_actor(
        ...
        self._logger.store({'Loss/Loss_total': loss.mean().item()})
