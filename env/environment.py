                ...
                for v in self.serve_UEs:
                    for i in range(v.n_layer):
                        if v == u and i == l:
                            ...
                            interference += (self.P_user[v.id][i] * v.combiner_eff_gain[i]* np.sqrt(self.Mt))
                        else:
...
class Environment:
    ...
    def reset(self, slot_bias=0):
        ...
        for k in range(self._K):
            self.Rt[k] = np.zeros((self._Mt, self._Mt), dtype=np.complex128)
            u_pos[k] = (np.random.rand(2) - 0.5) * self._region_bound
            for m in range(self._Mt):
                for n in range(self._Mt):
                    theta_bar = self.calculate_AOD(0, 0, u_pos[k][0], u_pos[k][1])
                    self.Rt[k][m][n] = 1 / self._Mt * np.exp(
                        -1j * self._kai * (m - n) * np.cos(theta_bar)) * np.sinc(
                        self._kai * (m - n) * self._delta_theta * np.sin(theta_bar))

        self.Rr = {}
        for k in range(self._K):
            self.Rr[k] = np.zeros((self._Mr, self._Mr), dtype=np.complex128)
            for m in range(self._Mr):
                for n in range(self._Mr):
                    theta_bar = self.calculate_AOD(0, 0, u_pos[k][0], u_pos[k][1])
                    self.Rr[k][m][n] = 1 / self._Mr * np.exp(
                        -1j * self._kai * (m - n) * np.cos(theta_bar)) * np.sinc(
                        self._kai * (m - n) * self._delta_theta * np.sin(theta_bar))
        ...
        if self.fix_channel:
            ...
            Hs = {}
            for k in range(self._K):
                dub = get_dis(u_pos[k], np.zeros(2), self._h_U, self._h_BS)
                self.Hl[k] = np.sqrt(self.get_largescale(dub, self._fc))
                Hs[k], _, _ = self.generate_gaussian_channel(self.Rr[k], self.Rt[k], self.UEs[k].n_layer)
                self._H[k] = self.Hl[k] * Hs[k]
        ...
        tensor_OLLA = torch.Tensor([olla for olla in self.BSs[0].OLLA.values()])
    ...
    def step(self, action: torch.Tensor):
        ...
        # 奖励
        # 直接以总传输bit数为奖励
        reward = tot_bits / 10 / self._K
        # reward = sum([b for u, b in enumerate(user_bits)]) / self._K
        ...
        if self.fix_channel:
            ...
            for k in range(self._K):
                Hs[k], _, _ = self.generate_gaussian_channel(self.Rr[k], self.Rt[k], self.UEs[k].n_layer)
                self._H[k] = self._rho * self._H[k] + self._rho2 * self.Hl[k] * Hs[k]
    ...
    def get_rate(self, u: User, mcs: str):
        ...
        if sinr >= self.MCS_table[mcs][1]:
            ACK = 1
            bits = self.MCS_table[mcs][0] * u.n_layer   # TODO: 实际传输的bits应当为：MCS谱效*带宽，这里由于后续需要单位归一化，直接使用谱效来计算
        else:
            ACK = 0
            bits = 0
        info["sinr"] = sinr
    ...
    def get_obs_dim(self):
        ...
        # 加上OLLA
        # return self._K_BS * self._M * 2 + self._K_BS * self._buffer_len + 1 + self._K_BS
        # return self._K_BS * self._N_rf * 2 + self._K_BS * self._buffer_len + 1 + self._K_BS
        # TODO: 尝试取消信道输入，只决策SINR估计偏移量
        return self._K * self._buffer_len + 1 + self._K
